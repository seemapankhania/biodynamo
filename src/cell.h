...
template <typename Base = SimulationObject<>,
  ...
  public:
    BDM_CLASS_HEADER_ADV(CellExt,
     ...
     neighbors_, biology_modules_, celltype_ID_);
  ...
  /// @see \link simulation_object_util.h Divide \endlink
    ...
    int GetCellProperty(string prop) { return cell_properties_[prop]; }
    int GetCellTypeID() const { return celltype_ID_[kIdx]; }
    ...
    void SetCellTypeID(int celltype_ID) {
      celltype_ID_[kIdx] = celltype_ID;
    }
    ...
    void SetCellProperty(string prop, int val) {
    // setting a cell property to a specific value
      cell_properties_[prop]=val;
    }
  ...
  /// information on cell
    map<string, int> cell_properties_;
    vec<int> celltype_ID_;
...
template <typename T, typename TBiologyModuleVariant>
  ...
  // Roman: cell type specification:
        double rand_double = ((double) rand() / (RAND_MAX));
        if (rand_double < 0.7) {
           // celltype_ID_[kIdx] = 1;
           daughter->SetCellTypeID(1);
         }
         else 
         {
           // celltype_ID_[kIdx] = 2;
           daughter->SetCellTypeID(2);
         }
      }
